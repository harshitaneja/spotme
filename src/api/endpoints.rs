...
pub async fn get_or_refresh_token(client_id: &str, client_secret: &str, redirect_uri: &str) -> Result<String> {
...
pub async fn start_librespot_daemon(token: String, mut receiver: mpsc::Receiver<LocalPlayerCommand>) -> Result<()> {
...
pub async fn play_track(token: &str, uri: &str, position_ms: u64) -> Result<(), anyhow::Error> {
    ...
    match req_res {
        Ok(r) => { 
            let _ = std::fs::write(&config::paths().log_file, format!("Play request sent. Status: {}, URL: {}\n", r.status(), url)); 
        }
        Err(e) => { 
            let _ = std::fs::write(&config::paths().log_file, format!("Play request FAILED: {}\n", e)); 
        }
    }
    Ok(())
}
...
pub async fn add_track_to_playlist_api(token: &str, playlist_id: &str, track_uri: &str) -> Result<(), anyhow::Error> {
    let client = crate::api::get_client();
...
pub async fn fetch_lyrics_api(track_name: &str, artist_name: &str) -> Result<Lyrics, anyhow::Error> {
...
pub async fn set_volume(token: &str, percent: u8) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let client = crate::api::get_client();
