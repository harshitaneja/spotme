...
pub fn handle_key_events(key: KeyEvent, app_state: &mut AppState, tx: &UnboundedSender<AppMessage>) -> anyhow::Result<bool> {
                ...
                match app_state.current_view {
                    ...
                    View::SelectPlaylist { ref track_uri, track_name: _, ref mut state, previous: _ } => {
                        match key.code {
                            KeyCode::Esc | KeyCode::Backspace | KeyCode::Char('b') => {
                                if let View::SelectPlaylist { previous, .. } = std::mem::replace(&mut app_state.current_view, View::Playlists) {
