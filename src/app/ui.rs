...
pub fn ui(f: &mut Frame, state: &mut AppState) {
    ...
    let fullscreen_popup = state.fullscreen_player && state.show_popup;
    let mut fullscreen_popup_area: Option<ratatui::layout::Rect> = None;
    ...
    if fullscreen_popup {
        ...
        fullscreen_popup_area = Some(popup_area);
    }
    ...
    if let Some(player) = &state.player_state {
        ...
        // In fullscreen: horizontal split — album art (left, full height) | info+lyrics (right)
        // In normal: horizontal split — small art | info
        let (art_area, right_area, lyrics_area) = if state.fullscreen_player {
            ...
            if has_lyrics {
                let v_split = Layout::default()
                    ...
                    .split(h_split[1]);
                (h_split[0], v_split[0], Some(v_split[1]))
            } else {
                (h_split[0], h_split[1], None)
            }
        } else {
            let split = Layout::default()
                ...
                .split(inner_area);
            (split[0], split[1], None)
        };

        let sub_chunks = vec![art_area, right_area];
            ...
            if let Some(lyrics) = &player.lyrics {
                if let Some(synced) = &lyrics.synced {
                    ...
                    if state.lyrics_mode == LyricsMode::Focused {
                        // 3-line focused view: previous, current, next
                        let lines_to_show = if active_idx == 0 { 2 } else { 3 };
                        let pad_top =
                            inner_lyrics_area.height.saturating_sub(lines_to_show) / 2;
                        for _ in 0..pad_top {
                            lyric_spans.push(Line::from(vec![Span::raw(" ")]));
                        }

                        // Previous line (grey, dimmed)
                        if active_idx >= 1 {
                            let text = &synced[active_idx - 1].text;
                        ...
                        // Next line (grey, dimmed)
                        if active_idx + 1 < synced.len() {
                            let text = &synced[active_idx + 1].text;
                            lyric_spans.push(Line::from(vec![Span::styled(
                                if text.is_empty() { " " } else { text }.to_string(),
                                Style::default().fg(Color::DarkGray),
    ...
    // Render view overlay AFTER player so popups appear on top in fullscreen
    if fullscreen_popup {
        if let Some(pa) = fullscreen_popup_area {
